    ...
    if (count > 0) {
      ...
      const promises = schedules.map(async schedule => {
        try {
          ...
          logger.info(`[SendScheduledMessage] Job adicionado à fila para: ${schedule.contact.name} em ${delay}ms`);
          logger.info(`Disparo agendado para: ${schedule.contact.name} em ${delay}ms`);
        } catch (err) {
          logger.error(`Erro ao agendar disparo para ${schedule.contact.name}: ${err.message}`);
